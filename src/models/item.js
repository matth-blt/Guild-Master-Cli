...
export class Item {
    ...
    // Vérifie si l'item peut être utilisé par un aventurier
    estUtilisablePar(classe, niveau) {
        if (niveau < this.conditions.niveauMinimum) {
            return false;
        }
        if (!this.conditions.toutesClasses && this.conditions.classeRequise !== classe) {
            return false;
        }
        return true;
    }
...
export const ItemTemplates = {
    ...
    epeeFer: () => new Item({
        nom: 'Épée de Fer',
        type: TypeItem.ARME,
        bonus: { attaque: 8, critique: 2 },
        rarete: Rarete.COMMUN
    }),
    epeeAcier: () => new Item({
        nom: 'Épée d\'Acier',
        type: TypeItem.ARME,
        bonus: { attaque: 15, critique: 5 },
        rarete: Rarete.RARE
    }),
    dagueSombre: () => new Item({
        nom: 'Dague de l\'Ombre',
        type: TypeItem.ARME,
        bonus: { attaque: 10, vitesse: 5, critique: 10 },
        rarete: Rarete.RARE,
        conditions: { classeRequise: Classe.VOLEUR, toutesClasses: false }
    }),
    batonElementaire: () => new Item({
        nom: 'Bâton Élémentaire',
        type: TypeItem.ARME,
        bonus: { attaque: 20 },
        rarete: Rarete.EPIQUE,
        conditions: { classeRequise: Classe.MAGE, toutesClasses: false }
    }),
    lameLegende: () => new Item({
        nom: 'Lame Légendaire',
        type: TypeItem.ARME,
        bonus: { attaque: 35, critique: 15, vitesse: 5 },
    ...
    cotteMailles: () => new Item({
        nom: 'Cotte de Mailles',
        type: TypeItem.ARMURE,
        bonus: { defense: 12 },
        rarete: Rarete.RARE
    }),
    armurePlaques: () => new Item({
        nom: 'Armure de Plaques',
        type: TypeItem.ARMURE,
        bonus: { defense: 25 },
        rarete: Rarete.EPIQUE,
        conditions: { classeRequise: Classe.TANK, toutesClasses: false }
    }),

    // Accessoires
    anneauSimple: () => new Item({
        nom: 'Anneau Simple',
        type: TypeItem.ACCESSOIRE,
        bonus: { attaque: 2, defense: 2 },
        rarete: Rarete.COMMUN
    }),
    amuletteChance: () => new Item({
        nom: 'Amulette de Chance',
        type: TypeItem.ACCESSOIRE,
        bonus: { critique: 8, esquive: 5 },
        rarete: Rarete.RARE
    }),
    bottesVent: () => new Item({
        nom: 'Bottes du Vent',
        type: TypeItem.ACCESSOIRE,
        bonus: { vitesse: 15, esquive: 8 },
        rarete: Rarete.EPIQUE
    })
};
